...
TOOL_CATALOG = {
    ...
    "query_by_trait": "Found traits extracted in the query, it could be expanded by running this tool. Searches for genetic associations with diseases, traits, or phenotypes mentioned in the user query and expands the gene list with additional related genes found through GWAS associations. Consider using this tool ONLY when traits are detected in the query are REQUIRED to answer the question, potentially discover more relevant genes, especially when the current gene list is small or when broader genetic context would be valuable for the analysis.",
