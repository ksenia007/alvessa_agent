...
from config import DEBUG, TOOL_PROT_MAX_GENES
from gene_class import Gene

# --- Local storage layout ---
ROOT = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(ROOT, "local_dbs", "alvessa_proteins.db")
PDB_DIR = os.path.join(ROOT, "local_dbs", "pdb")
STATIC_FOLDER = "static"
HTML_TEMPLATE = os.path.join(ROOT, STATIC_FOLDER, "tool_prot.html")
CSS_TEMPLATE = os.path.join(ROOT, STATIC_FOLDER, "tool_prot.css")
JS_TEMPLATE = os.path.join(ROOT, STATIC_FOLDER, "tool_prot.js")
...
if __name__ == "__main__":
    ...
    state = State({
        "genes": genes,
        "gene_entities": {g: Gene(symbol=g) for g in genes}
    })
